...
$.ajax({    
    ...
    success: function(data) {
        ...
        let counterparty_info_str = counterparty_info['abbreviation']+" "+counterparty_info['counterparty']+" "+counterparty_info['taxpayer_id_number'];
        document.getElementById('counterparty_info').innerHTML = counterparty_info_str;
...
$("#invoices").on("click", function(){
    if(role_selected == "partner_warehouse"){
        invoicesForPartner('partner'); 
    }else{
        invoicesForPartner('provider');
    }       
});
...
function allert(){
    console.log("test button "+counterparty_info['counterparty_id']+' '+counterparty_info['companyInfoString']);    
}
function openPDF(){
    myCreatePDF();
...
function invoicesForPartner(warehouse_type){
    ...
    $.ajax({
        ...
        data: {'receive_partner_invoices_list':0
                , 'counterparty_id': counterparty_info['counterparty_id'] 
                , 'document_name':'товарная накладная','warehouse_type': warehouse_type},
        dataType: 'html',
        beforeSend: function() {
            //ожидание пока функция выполняется кнопка не активна
...
//список заказов поставщика
function ordersForProvider(){
    $.ajax({
        //создаем url для отправки в .php
        url: 'partner_PDF.php',
        type: 'POST',
        cache: false,
        data: {'receive_provider_orders_list':0
              ,'counterparty_id': counterparty_info['counterparty_id'] },
        dataType: 'html',
        beforeSend: function() {
            //ожидание пока функция выполняется кнопка не активна
...
function getProviderOrderThis(id_str){
    $.ajax({
        //создаем url для отправки в .php
        url: 'partner_PDF.php',
        type: 'POST',
        cache: false,
        data: {'receive_order_partner_this':0, 'order_partner_id':id_str },
        dataType: 'html',
        beforeSend: function() {
            //ожидание пока функция выполняется кнопка не активна
            //$("#orders").prop("disabled", true);
...
function getProviderInvoceThis(doc_num, invoice_key_id){
    $.ajax({
        //создаем url для отправки в .php
        url: 'partner_PDF.php',
        type: 'POST',
        cache: false,
        data: {'receive_invoice_provider_this':0, 'doc_num':doc_num, 'invoice_key_id':invoice_key_id },
        dataType: 'html',
        beforeSend: function() {
            //ожидание пока функция выполняется кнопка не активна
            //$("#orders").prop("disabled", true);
            ...
            elems[i].addEventListener('click', function(){
                ...
                if(id_str.length != 0){
                    ...
                    if(role_selected === 'partner_warehouse'){
                        ...
                        getProviderInvoceThis(list[id_str].document_num, list[id_str].invoice_key_id);
                    }
...
function createTable(sheet_data){    
    ...
    if(sheet_data.length > 0)
    {
        ...
        table_output +='<tr>';
            ...
            arr_str = ['Отгружен', 'Дата', 'Номер','Склад','Компания','Склад','Сумма','Дата отгрузки'];
            arr.push(arr_str);

        for(var row = 0; row < sheet_data.length; row++)
        {
            arr_str = [sheet_data[row]['executed']
                        ,sheet_data[row]['created_date']
                        ,sheet_data[row]['order_id']
                        ,sheet_data[row]['out_warehouseInfoString']
                        ,sheet_data[row]['in_companyInfoString_short']
                        ,sheet_data[row]['in_warehouseInfoString']
                        ,sheet_data[row]['order_summ']
                        ,sheet_data[row]['get_date']];
            arr.push(arr_str);
            ...
            table_output +='<td><div id="'+(row)+'" class="inputs"  type="text">'+sheet_data[row]['out_warehouseInfoString']+'</div></td>';               
            table_output +='<td><div id="'+(row)+'" class="inputs"  type="text">'+sheet_data[row]['in_companyInfoString_short']+'</div></td>'; 
            table_output +='<td><div id="'+(row)+'" class="inputs"  type="text">'+sheet_data[row]['in_warehouseInfoString']+'</div></td>'; 
            ...
            elems[i].addEventListener('click', function(){
                ...
                if(id_str.length != 0){
                    ...
                    if(role_selected === 'partner_warehouse'){
                        ...
                        getProviderOrderThis(sheet_data[id_str].order_id);
                    }
...
function createThisOrderTable(list){
    ...
    if(list.length > 0)
    {
        ...
        for(var row = 0; row < order_list.length; row++)
        {
            ...
            table_output +='<tr>';
            table_output +='<td><div id="'+(row)+'" class="inputs"  type="text">'+(row+1)+'</div></td>';
