<?php
   ...
   //check_inventory_of_goods.php?check_inventory_start
    ...
    if(isset($_GET['check_inventory_start'])){     
        ...
        //начать исправление запасов товар у поставщика
        $taxpayer_id_number = start_check_inventory_002($con);
        ...
        clear_table_t_check_inventory_of_goods($con);
    ...
    function which_position_was_not_found($con, $taxpayer_id_number){
        ...
        foreach($id_false_list as $k => $id_false){
            ...
            if($my_flag == false){
                echo "В ЛИСТЕ ЗАПАСЫ НЕТ ЭТОГО ТОВАРА / my_id = $product_inventory_id name =  $in_product_name<br>";                
    ...
    function start_check_inventory_002($con){
        $taxpayer_id_number = 0;
        $percent = $GLOBALS['percent_of_loading_goods_to_warehouse'];
        //$procent = 0.7;
        ...
        if(mysqli_num_rows($result) > 0){  
            ...
            echo "начальные данные не получены, проверьте данные в первой строке таблицы t_check_inventory_of_goods";
            return;
        }
        ...
        if(mysqli_num_rows($res) > 0){ 
            ...
            //данных нет вернуться
            echo "в t_inventory_vs_inproductname нет товаров этого поставщика, перепроверьте данные и попробуйте еще раз";
            return;
        }
            ...
            if(mysqli_num_rows($res) > 0){
                ...
                //проверить в какую сторону провести изменения добавить товара или уменьшить
                    ...
                    if($quantity_finish >= $min_sell){
                        ...
                        $make_reserve = $quantity_finish * $percent;
                    ...
                    if($make_reserve == 0){
                        ...
                        if($provider_stock_quantity > 0){
                            ...
                            echo "ЗАПАС  изменен на 0 <br>";
                        }
                        else{
                            echo "ОСТАТОК НА СКЛАДЕ РАВЕН = 0; (или отрицательный), действие уменьшить не выполнялось <br>";
                        }
                        ...
                        $query="INSERT INTO `t_warehouse_inventory_in_out`
                            ...
                            VALUES ('return'             ,'$product_inventory_id','$quantity','$provider_warehouse_id',  '1'    ,   '1'   )";
                            mysqli_query($con, $query) or die (mysqli_error($con));
                            echo "ТОВАР уменньшен <br>";
                    }else{
                        echo "ЗАПАСЫ РАВНЫ <br>";
                    }
            
            
            }else{
                //товар не найден в новом списке от поставщика
                ...
                mysqli_query($con, $query) or die (mysqli_error($con));
                echo "В списке поставщика этот товар отсутствовал, поэтому ТОВАР уменньшен до $my_in_product_name = 0<br>";
                
                }else{
                    echo "В списке поставщика этот товар отсутствовал,  ТОВАР $my_in_product_name был равен >= 0<br>";
                }
            }           
    ...
    /*function start_check_inventory($con){
        ...
        if(mysqli_num_rows($result) > 0){        
            ...
            foreach($id_list as $k => $id){
                ...
                if(mysqli_num_rows($res) > 1){ 
                    ...
                    while($row = mysqli_fetch_array($res)){
                        ...
                        echo $in_product_name . "id= $id / product_inventory_id= " . $product_inventory_id . "<br>";
                    }
                   ...
                   // $min_sell = 1;
                    ...
                    if($make_reserve == 0){
                        ...
                        if($provider_stock_quantity > 0){
                            ...
                            echo "ОСТАТОК НА СКЛАДЕ РАВЕН = 0; ИЛИ ОТРИЦАТЕЛЬНЫЙ действие уменьшить отменено <br>";
                        }
            ...
            echo "нет информации в таблице <br>";
        }
        return $taxpayer_id_number;
    }*/
